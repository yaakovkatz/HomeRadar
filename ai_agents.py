"""
ai_agents.py - AI Agents for post classification and data extraction
Agent 1: Classification (RELEVANT/SPAM/BROKER/etc) - with image support
Agent 2: Extraction (fill missing price/city)
"""

import os
import json
from anthropic import Anthropic
from dotenv import load_dotenv
import time


# טעינת API Key מ-.env
load_dotenv()


class AIAgents:
    def __init__(self):
        """Initialize AI Agents with Anthropic API"""
        api_key = os.getenv('ANTHROPIC_API_KEY')
        if not api_key:
            raise ValueError("❌ ANTHROPIC_API_KEY לא נמצא בקובץ .env")

        self.client = Anthropic(api_key=api_key)
        self.model = "claude-sonnet-4-20250514"  # Sonnet 4

        self.last_call = 0
        self.min_delay = 0.5  # 500ms בין קריאות

        print("✅ AI Agents initialized successfully")

    def _wait_if_needed(self):
        """ממתין אם עברו פחות מ-500ms מהקריאה האחרונה"""
        now = time.time()
        elapsed = now - self.last_call
        if elapsed < self.min_delay:
            time.sleep(self.min_delay - elapsed)
        self.last_call = time.time()

    # =========================================================
    # Agent 1: Classification (סינון) - עם תמיכה בתמונות
    # =========================================================

    def classify_post(self, content, author, images=None, group_name=None):
        """
        Agent 1: מסווג פוסט לקטגוריה (עם תמיכה בתמונות!)

        Args:
            content: תוכן הפוסט
            author: שם המפרסם
            images: רשימת URLs של תמונות (אופציונלי)
            group_name: שם קבוצת הפייסבוק (אופציונלי)

        Returns:
            {
                'category': 'RELEVANT' | 'BROKER' | 'SPAM' | 'AUCTION' | 'WANTED' | 'QUESTION' | 'NON_URBAN' | 'SUSPECTED_BROKER' | 'ROOMMATE',
                'is_broker': True/False,
                'confidence': 0.0-1.0,
                'reason': 'הסבר קצר'
            }
        """

        self._wait_if_needed()

        prompt = f"""אתה מומחה לסיווג פוסטים בקבוצות פייסבוק של דירות יד שנייה.

{"קרא את התמונות והפוסט" if images else "קרא את הפוסט הבא"} וסווג אותו לאחת מהקטגוריות:

**RELEVANT** - דירה למכירה/השכרה (רלוונטי!)
- יש תיאור דירה (חדרים, מטר, קומה וכו')
- יכול להיות עם או בלי מחיר
- יכול להיות ממתווך או מבעלים
- **חשוב: רק בערים הרלוונטיות!** (ראה NON_URBAN למטה)

**NON_URBAN** - מושב/כפר/יישוב/עיר לא רלוונטית
- דירה/בית במושב, כפר, יישוב קהילתי, התנחלות וכו'
- **או עיר שלא ברשימה הרלוונטית!**
- **רק ערים רלוונטיות:** ירושלים, מעלה אדומים, בית שמש, בני ברק, רמת גן, גבעתיים, אשדוד
- **ערים לא רלוונטיות - סנן מיד:**
  - תל אביב → NON_URBAN (לא ברשימה)
  - חיפה → NON_URBAN (לא ברשימה)
  - רמת השרון → NON_URBAN (לא ברשימה)
  - פתח תקווה → NON_URBAN (לא ברשימה)
  - נתניה → NON_URBAN (לא ברשימה)
- **יישובים קטנים - סנן מיד:**
  - "נופים" (יישוב בבית שמש) → NON_URBAN ⚠️ חשוב!
  - "נוף איילון" → NON_URBAN
  - "מצפה נטופה" → NON_URBAN
  - "כפר אדומים" → NON_URBAN
  - "אלון שבות" → NON_URBAN
  - "אפרת" → NON_URBAN
  - "מושב צור משה" → NON_URBAN
  - "יישוב קהילתי X" → NON_URBAN
  - "התנחלות Y" → NON_URBAN
- **שכונות ידועות - דוגמאות:**
  - **ירושלים:** נחלאות, מאה שערים, גאולה, קטמון, רמות, תלפיות, ארנונה, פסגת זאב, גבעת שאול, קריית יובל, רוממה, בית וגן, גילה, מורדות גילה, German Colony
  - **בית שמש:** נווה שמיר, רמת בית שמש, הר טוב
  - **בני ברק:** פרדס כץ, עזרא, גבעת רוקח

**🔥 כלל זהב (חשוב מאוד!):**
1. **אם שם הקבוצה מכיל "ירושלים" / "בית שמש" / "בני ברק" / "רמת גן" / "גבעתיים" / "אשדוד" / "מעלה אדומים":**
   - → כל שכונה/מיקום באותה עיר = **RELEVANT** (גם אם לא מכיר את השם!)
   - → סנן רק אם יש מילות מפתח מפורשות: "מושב", "יישוב", "כפר", "התנחלות"
   - דוגמאות:
     * קבוצה: "דירות בירושלים" + מיקום: "מורדות גילה" → **RELEVANT** ✅
     * קבוצה: "דירות בירושלים" + מיקום: "מושב צור הדסה" → **NON_URBAN** ❌ (יש "מושב")

2. **אם שם הקבוצה לא מכיל עיר:**
   - → בדוק אם המיקום בפוסט הוא עיר רלוונטית
   - אם כן → **RELEVANT**
   - אם לא → **NON_URBAN**

- **זיהוי חכם:** אם הטקסט מכיל "מושב", "יישוב", "התנחלות", "כפר" **במפורש** → NON_URBAN
- **חשוב!** ב-reason תמיד ציין: "[שם המיקום] - [סיבה]" (למשל: "רמת השרון - עיר לא ברשימה")

**AUCTION** - מכרז/כונס נכסים/מכירה פומבית
- תמונה או טקסט עם "כונס נכסים", "מכרז", "הזמנה להציע הצעות"
- מכירה פומבית, רכישת זכויות

**SUSPECTED_BROKER** - דירה רלוונטית עם חשד למתווך 🟡
- **יש תיאור דירה ספציפית** (כמו RELEVANT)
- אבל יש סימנים חשודים שזה אולי מתווך:
  * שם חברה: "מור נכסים", "אריה נכסים", "[שם] real estate"
  * מבנה מקצועי מדי (אבל ללא מספר רישיון!)
  * טון עסקי חזק (אבל לא מספיק ברור)
  * **ביטויי שיווק נדל"ן:** "נכס זה בבלעדיות", "בלעדי!", "נכס בלעדי"
  * **הזמנה לסיור:** "לסיור בנכס", "תיאום סיור", "לתיאום צפייה", "לצפייה בנכס"
  * **ביטויים דומים:** כל משפט שמזמין ל"סיור", "צפייה", "ביקור" בנכס עם טלפון
- **לא מספיק ראיות חזקות** לסווג כ-BROKER וודאי
- **דוגמאות:**
  * "דירה 4 חדרים... מור נכסים 054-1234567" → SUSPECTED_BROKER
  * "להשכרה בבני ברק... נכסים פלוס ליאור" → SUSPECTED_BROKER
  * "נכס זה בבלעדיות... לסיור בנכס חייגו" → SUSPECTED_BROKER
  * "לתיאום צפייה: 050-1234567" → SUSPECTED_BROKER
- **המטרה:** לתת למשתמש להחליט אם להוסיף לרשימה שחורה

**BROKER** - מתווך מחפש לקוחות (לא דירה ספציפית!)
- כתוב "מחפש לקוחות", "תיק נכסים", "שירות תיווך"
- אין תיאור דירה ספציפית

**זיהוי מתווכים סמויים (חשוב!):**
מתווכים לפעמים **מסתירים** שהם מתווכים. שים לב ל:
- ✅ **מספר רישיון מתווך** (הכי חשוב!):
  * "מספר רישיון: 1234567", "מ.ר. 1234567", "ברישיון 1234567"
  * **אם יש מספר רישיון → זה 100% מתווך!**
- ✅ חתימה עסקית: "נדל״ן JF", "Real Estate X", "נכסים פלוס", "מקסים נדל״ן", "דורון נכסים"
- ✅ מספר נכסים באותו פוסט (יותר מדירה אחת)
- ✅ טון עסקי + חתימה מקצועית: "לפרטים ותיאום: שירה 050-1234567"
- ✅ ריבוי אייקונים פרסומיים (✨📍💎🏠) + עיצוב מסחרי
- ✅ כותרת מסחרית: "עסקת פרימיום!", "הזדמנות נדירה!", "בלעדי!"

**אבל שים לב:**
- ❌ אדם פרטי שכותב טוב ≠ מתווך
- ❌ שימוש באמוג'ים בלבד ≠ מתווך
- ❌ פוסט מסודר עם פרטים ≠ מתווך

**אם זה מתווך סמוי:**
→ category: "RELEVANT", is_broker: true, reason: "מתווך (חתימה עסקית: נדל״ן X)"

**SPAM** - פרסום לא קשור
- שיפוצים, נקיון, הובלות (בתמונה או בטקסט)
- לא קשור לדירות

**WANTED** - מחפש דירה (לא מוכר!)
- "מחפש דירה", "דרוש", "צריך"

**ROOMMATE** - חיפוש שותף/סאבלט ❌
- **סאבלט/תת-השכרה:** "מסאבלט", "תת השכרה", "להשכיר את הדירה שלי"
- **חיפוש שותפים:** "מחפש שותף", "מחפשת שותפה", "שותף לדירה"
- **הצעת חדר:** "חדר פנוי", "חדר להשכרה בדירה משותפת"
- **דוגמאות:**
  * "מסאבלט דירה לסופ״ש" → ROOMMATE
  * "מחפשת שותפה לדירה בנחלאות" → ROOMMATE
  * "חדר פנוי בדירה 4 חדרים" → ROOMMATE

**QUESTION** - שאלה
- "מישהו יודע?", "איך...?", "עזרה"

---

**פוסט:**
מפרסם: {author}
{"קבוצה: " + group_name if group_name else ""}
תוכן: {content[:500]}
{"תמונות: מצורפות (קרא אותן!)" if images else ""}

---

**חשוב - תמונות:**
- אם יש תמונה עם "כונס נכסים", "מכרז", "הזמנה להציע הצעות" → AUCTION
- אם יש תמונה של פרסום (הובלות, שיפוצים, נקיון) → SPAM
- קרא את התמונות בקפידה!

**חשוב - תגובות:**
- הטקסט עלול להכיל תגובות של אנשים אחרים (כמו "משה: תגובתי מאוד!")
- **התעלם לגמרי מתגובות!**
- התמקד **רק בתוכן המקורי** של הפוסט
- תגובות מתחילות לרוב בשם אדם + "תגובתי", "מה זה", "יקר מדי" וכו'
- אם יש תיאור דירה בפוסט המקורי → RELEVANT (גם אם יש תגובה שאלה!)

---

**החזר תשובה ב-JSON בדיוק בפורמט הזה (ללא טקסט נוסף):**
{{
    "category": "RELEVANT",
    "is_broker": false,
    "confidence": 0.95,
    "reason": "דירה למכירה עם תיאור מפורט"
}}

**כללים:**
1. **סאבלט/שותפים** - "מסאבלט", "תת השכרה", "מחפש שותף" → category: "ROOMMATE" ⚠️ חשוב!
2. **מתווך וודאי** (מספר רישיון / חתימה מפורשת) → category: "RELEVANT", is_broker: true
3. **חשד למתווך** (שם חברת נכסים / מבנה עסקי אבל לא ברור) → category: "SUSPECTED_BROKER", is_broker: false
4. אם מתווך מחפש לקוחות (לא דירה ספציפית) → category: "BROKER", is_broker: true
5. אם מכרז/כונס נכסים → category: "AUCTION", is_broker: false
6. confidence: 0.5-1.0 (עד כמה אתה בטוח)
7. אם confidence < 0.5 → category: "RELEVANT" (במקרה ספק)
8. **שם חברה כמו "מור נכסים", "אריה נכסים"** → SUSPECTED_BROKER (לא RELEVANT!)

**חשוב - פורמט ה-reason:**
- **תמיד ציין את המיקום שזיהית!**
- פורמט: "[מיקום מזוהה] - [סיבת הסינון]"
- דוגמאות:
  * NON_URBAN: "רמת השרון - עיר לא ברשימה הרלוונטית"
  * NON_URBAN: "German Colony, Jerusalem - שכונה לא מוכרת (אין ברשימת השכונות)"
  * SUSPECTED_BROKER: "בני ברק - חשד למתווך (מור נכסים)"
  * RELEVANT: "ירושלים, נחלאות - דירה רלוונטית"

**דוגמאות:**

✅ מתווך סמוי (מספר רישיון):
"דירת 5 חדרים במיקום מעולה!
לפרטים: דורון נכסים 054-9388399
מספר רישיון: 3205091"
→ category: "RELEVANT", is_broker: true, reason: "מתווך (מספר רישיון: 3205091)"

✅ מתווך סמוי (מ.ר.):
"דירה ענקית! המון פוטנציאל!
לפרטים: אסף 050-2262874
מ.ר. 3243938"
→ category: "RELEVANT", is_broker: true, reason: "מתווך (מ.ר. 3243938)"

✅ מתווך סמוי (חתימה):
"דירת 4 חדרים מדהימה ✨ במיקום פרימיום 📍
לפרטים ותיאום: סתיו - נדל״ן JF 📞 054-7895735"
→ category: "RELEVANT", is_broker: true, reason: "מתווך (חתימה: נדל״ן JF)"

✅ מתווך סמוי (מספר נכסים):
"💎 3 דירות חדשות בתל אביב! 🏠
Real Estate Plus | שירה 050-1234567"
→ category: "RELEVANT", is_broker: true, reason: "מתווך (מספר נכסים + Real Estate Plus)"

🟡 חשוד למתווך (שם חברה):
"למכירה ברחוב השניים בבני ברק
דירת 4 חדרים - מרווחת ומוארת - 85 מ״ר
לפרטים נוספים - ליאור - 0545477042
מור נכסים"
→ category: "SUSPECTED_BROKER", is_broker: false, reason: "בני ברק - חשד למתווך (מור נכסים)"

🟡 חשוד למתווך (פורמט עסקי):
"דירה 5 חדרים בירושלים 🏠 מיקום מעולה ✨
אריה נכסים - 054-1234567"
→ category: "SUSPECTED_BROKER", is_broker: false, reason: "ירושלים - חשד למתווך (אריה נכסים)"

🟡 חשוד למתווך (real estate):
"3 rooms apartment in Beit Shemesh
For details: Dan Real Estate 052-5555555"
→ category: "SUSPECTED_BROKER", is_broker: false, reason: "בית שמש - חשד למתווך (Dan Real Estate)"

🚫 עיר לא רלוונטית:
"דירה 4 חדרים ברמת השרון
3,500₪ לחודש, מעלית וחניה"
→ category: "NON_URBAN", is_broker: false, reason: "רמת השרון - עיר לא ברשימה הרלוונטית"

🚫 מושב (גם אם בקבוצת ירושלים):
"קבוצה: דירות בירושלים
דירה 5 חדרים במושב צור הדסה
מיקום מעולה, גינה פרטית"
→ category: "NON_URBAN", is_broker: false, reason: "מושב צור הדסה - יישוב לא רלוונטי (מושב)"

✅ שכונה בירושלים (גם אם לא מוכרת!):
"קבוצה: דירות להשכרה בירושלים
דירת 4 חדרים במורדות גילה
מרווחת ומוארת, 5 דקות ממלחה"
→ category: "RELEVANT", is_broker: false, reason: "ירושלים, מורדות גילה - דירה רלוונטית"

🚫 סאבלט (לא רלוונטי):
"מסאבלט את הדירה בנחלאות לסופ״ש הבא: 4-7.2
הדירה מהממת, יש לה גינה גדולה, מטבח מאובזר..."
→ category: "ROOMMATE", is_broker: false, reason: "נחלאות, ירושלים - סאבלט (לא רלוונטי)"

🚫 חיפוש שותף (לא רלוונטי):
"מחפשת שותפה לדירה 3 חדרים בבני ברק
קומה 2, מעלית, 1,500₪ לחודש"
→ category: "ROOMMATE", is_broker: false, reason: "בני ברק - חיפוש שותפה (לא רלוונטי)"

✅ דירה רלוונטית:
"דירה יפה להשכרה! 3 חדרים, מעלית, חניה 🏡
נחלאות, ירושלים
למעוניינים: יוסי 052-9876543"
→ category: "RELEVANT", is_broker: false, reason: "ירושלים, נחלאות - דירה רלוונטית"
"""

        try:
            # בניית message עם תמונות
            message_content = []

            # אם יש תמונות - הוסף אותן קודם
            if images:
                for image_url in images:
                    message_content.append({
                        "type": "image",
                        "source": {
                            "type": "url",
                            "url": image_url
                        }
                    })

            # הוסף את הטקסט
            message_content.append({
                "type": "text",
                "text": prompt
            })

            # שלח ל-Claude
            response = self.client.messages.create(
                model=self.model,
                max_tokens=200,
                temperature=0,
                messages=[{"role": "user", "content": message_content}]
            )

            # חילוץ התשובה
            result_text = response.content[0].text.strip()

            # ניקוי markdown backticks אם יש
            result_text = result_text.replace('```json', '').replace('```', '').strip()

            # המרה ל-JSON עם error handling
            try:
                result = json.loads(result_text)
            except json.JSONDecodeError as json_err:
                print(f"❌ Agent 1 - JSON parse error: {json_err}")
                print(f"   Raw response: {result_text[:100]}...")
                return {
                    'category': 'RELEVANT',
                    'is_broker': False,
                    'confidence': 0.5,
                    'reason': f'JSON parse failed'
                }

            # ולידציה
            if result['confidence'] < 0.5:
                result['category'] = 'RELEVANT'  # במקרה ספק

            return result

        except Exception as e:
            print(f"❌ Agent 1 failed: {e}")
            # ברירת מחדל: נניח שזה רלוונטי
            return {
                'category': 'RELEVANT',
                'is_broker': False,
                'confidence': 0.5,
                'reason': f'AI failed: {str(e)}'
            }

    # =========================================================
    # Agent 2: Extraction (חילוץ)
    # =========================================================

    def extract_missing_details(self, content, regex_found, group_name=None):
        """
        Agent 2: ממלא פרטים חסרים (מחיר, עיר, מיקום)

        Args:
            content: תוכן הפוסט
            regex_found: מה ש-Regex כבר מצא
                {
                    'price': '7200' | None,
                    'city': 'ירושלים' | None,
                    'location': 'פסגת זאב' | None,
                    'rooms': '4' | None,
                    'phone': '050...' | None
                }
            group_name: שם קבוצת הפייסבוק (אופציונלי) - לעזור בזיהוי עיר

        Returns:
            {
                'price': '7200' | None,
                'city': 'ירושלים' | None,
                'location': 'פסגת זאב' | None
            }
        """

        self._wait_if_needed()

        # מה חסר?
        missing = []
        if not regex_found.get('price'):
            missing.append('מחיר')
        if not regex_found.get('city'):
            missing.append('עיר')
        if not regex_found.get('location'):  # ← הוסף את זה!
            missing.append('מיקום (שכונה/רחוב)')

        if not missing:
            # אין מה למלא!
            return {'price': None, 'city': None, 'location': None}

        missing_str = ', '.join(missing)

        # הכנת מידע על קבוצה (אם יש)
        group_info = ""
        if group_name:
            group_info = f"\n    **קבוצת פייסבוק:** {group_name}"

        prompt = f"""אתה מומחה לחילוץ מידע מפוסטים בעברית.

    קרא את הפוסט הבא וחלץ את הפרטים החסרים: **{missing_str}**

    **פוסט:**
    {content[:800]}{group_info}

    ---

    **הנחיות קריטיות:**

1. **מחיר (מחיר הדירה בלבד!)**: 
   - חפש מחיר של הדירה עצמה
   - **התעלם ממחירים של:** מטבח, שיפוץ, ארונות, רהיטים, השקעה
   - דוגמאות לא נכונות: "מטבח 100,000", "שיפוץ 50,000"
   - אם אין מחיר דירה → null
   
   2. **עיר:**
   - חפש עיר מפורשת מהרשימה הרלוונטית: "ירושלים", "מעלה אדומים", "בית שמש", "בני ברק", "רמת גן", "גבעתיים", "אשדוד"
   - **ערים אחרות (תל אביב, חיפה, פתח תקווה וכו') לא רלוונטיות!**
   - **אם יש שכונה ידועה → הסק את העיר!**
     דוגמה: "דירה בקטמון" → city: "ירושלים" (כי קטמון זו שכונה בירושלים)
     דוגמה: "דירה בנווה שמיר" → city: "בית שמש" (כי נווה שמיר זו שכונה בבית שמש)
   - **שימוש בשם קבוצה (קדימות ראשונה!):**
     * **אם שם הקבוצה מכיל עיר רלוונטית** (ירושלים, בית שמש, בני ברק וכו') **→ זו העיר!**
     * **קריטי:** קבוצה "דירות בית שמש" + שכונה בפוסט → city: "בית שמש" (גם אם אתה לא מכיר את השכונה!)
     * דוגמה: קבוצה "דירות בבית שמש", פוסט "נווה שמיר" → city: "בית שמש" ✅
     * דוגמה: קבוצה "דירות בירושלים", פוסט "רחוב יצחק בן צבי" → city: "ירושלים" ✅
     * **חריג יחיד:** אם הפוסט מזכיר עיר אחרת **במפורש** → השתמש בעיר המפורשת (אם היא ברשימה הרלוונטית!)
   - **אם אין עיר מפורשת ואין שכונה ידועה ואין עיר מהקבוצה → null**
   
3. **מיקום (שכונה/רחוב) - קריטי!**
   - **זה השדה הכי חשוב - תמיד חפש אותו!**
   - חפש שכונה ו/או רחוב בפוסט
   
   **דוגמאות לשכונות:**
   - ירושלים: נחלאות, קטמון, קריית יובל, פסגת זאב, גבעת שאול, תלפיות, ארנונה, מאה שערים, גאולה, רמות, רוממה, בית וגן
   - בית שמש: נווה שמיר, רמת בית שמש, נופים, הר טוב
   - בני ברק: פרדס כץ, עזרא, גבעת רוקח
   - רמת גן: רמת אפעל, רמת יצחק, רמת חן
   
   **דוגמאות לרחובות:**
   - "רחוב הרצל", "רח' בן יהודה", "דרך בגין"
   
   **כללים:**
   - אם יש **רק שכונה** → החזר שכונה
   - אם יש **רק רחוב** → החזר רחוב
   - אם יש **גם שכונה וגם רחוב** → החזר את שניהם בפורמט: "שכונה, רחוב"
   - אם באמת אין → null

   **⚠️ לא לבלבל עם תיאורים כלליים:**
   - "מדרחוב", "מדרכוב", "מדרגות" → אלה **תיאורים של סוג רחוב**, לא שם מקום!
   - "מדרחוב" = pedestrian street (רחוב הולכי רגל) → אם כתוב "ממוקם על מדרחוב" → location: null
   - דוגמה: "ממוקמת על מדרכוב התוסס" → location: null (זה לא שכונה!)
   
   **דוגמאות:**
   - "דירה בקריית יובל, רחוב ז'בוטינסקי" → location: "קריית יובל, רחוב ז'בוטינסקי"
   - "דירה בקריית יובל" → location: "קריית יובל"
   - "דירה ברחוב הרצל" → location: "רחוב הרצל"
   - "דירה בירושלים" → location: null

   **זיהוי שכונה מרחוב מרכזי (חשוב!):**
   - אם יש רחוב מפורש + עיר, **נסה לזהות את השכונה** אם אתה בטוח
   - דוגמאות:
     * "רחוב סוקולוב בירושלים" → location: "גבעת שאול, רחוב סוקולוב"
     * "רחוב הרב קוק בירושלים" → location: "מרכז העיר, רחוב הרב קוק"
     * "רחוב ז'בוטינסקי בבני ברק" → location: "מרכז העיר, רחוב ז'בוטינסקי"
   - **רק אם אתה בטוח 100% ברחוב מרכזי שידוע לך בשכונה מסוימת!**
   - אם לא בטוח - החזר רק את הרחוב

         **חשוב:** גם אם אין עיר, אם יש רחוב - החזר אותו ב-location!
דוגמה: "דירה ברחוב הארזים 12" → city: null, location: "רחוב הארזים"         

    4. **כלל זהב: אם לא בטוח → null**

    ---

    **חשוב - תגובות:**
    - הטקסט עלול להכיל תגובות של אנשים אחרים
    - **חלץ מידע רק מהפוסט המקורי, התעלם לגמרי מתגובות!**
    - דוגמה: אם בפוסט כתוב "7000 שח" ובתגובה "אני מוכן 5000" → החזר "7000"
    - דוגמה: אם בפוסט כתוב "בירושלים" ובתגובה "יש גם בתל אביב?" → החזר "ירושלים"

    ---

    **דוגמאות חשובות:**

    ✅ "דירה בירושלים ברחוב הרצל"
       → city: "ירושלים", location: "רחוב הרצל"

    ✅ "דירה בפסגת זאב ברחוב אלי תבין"
       → city: "ירושלים", location: "פסגת זאב, רחוב אלי תבין" (שכונה ידועה → הסק עיר!)

    ✅ "דירה בקטמונים ברחוב יוסי בן יועזר"
       → city: "ירושלים", location: "קטמון, רחוב יוסי בן יועזר" (קטמון ידוע → הסק עיר!)

    ✅ קבוצה: "דירות בירושלים", פוסט: "דירה ברחוב יצחק בן צבי"
       → city: "ירושלים", location: "רחוב יצחק בן צבי" (הסק מהקבוצה!)

    ❌ "דירה ברחוב הרצל" (ללא קבוצה)
       → city: null, location: "רחוב הרצל" (אין שכונה ידועה - רק רחוב!)

    ❌ "דירה ברחוב בגין בשכונה יפה"
       → city: null, location: "רחוב בגין" ("שכונה יפה" לא שם שכונה אמיתי!)
       
               **דוגמאות מחיר:**
        
        ✅ "דירה למכירה 2,500,000 ש״ח" → price: "2500000"
        ✅ "מחיר מבוקש: 3.5 מיליון" → price: "3500000"
        ❌ "מטבח חדש 100,000 ש״ח" → price: null (זה מחיר מטבח!)
        ❌ "השקענו 200,000 בשיפוצים" → price: null (זה שיפוץ!)
        ❌ "ארונות בהתאמה 80,000" → price: null (זה ריהוט!)
        
        **אם יש גם מחיר דירה וגם מחירים של שדרוגים - קח רק את מחיר הדירה!**
        דוגמה: "דירה 2,800,000 ש״ח, מטבח 100,000" → price: "2800000"
        
            ---

    **החזר תשובה ב-JSON בדיוק בפורמט הזה (ללא טקסט נוסף):**
    {{
        "price": "7200",
        "city": "ירושלים",
        "location": "פסגת זאב"
    }}

    אם אין מידע → השתמש ב-null
    """

        try:
            response = self.client.messages.create(
                model=self.model,
                max_tokens=200,
                temperature=0,
                messages=[{"role": "user", "content": prompt}]
            )

            # חילוץ התשובה
            result_text = response.content[0].text.strip()

            # ניקוי markdown backticks
            result_text = result_text.replace('```json', '').replace('```', '').strip()

            # המרה ל-JSON עם error handling
            try:
                result = json.loads(result_text)
            except json.JSONDecodeError as json_err:
                print(f"❌ Agent 2 - JSON parse error: {json_err}")
                print(f"   Raw response: {result_text[:100]}...")
                return {'price': None, 'city': None, 'location': None}

            # ולידציה: רק מחזירים מה שהיה חסר
            output = {}

            if not regex_found.get('price'):
                output['price'] = result.get('price')

            if not regex_found.get('city'):
                output['city'] = result.get('city')

            if not regex_found.get('location'):
                output['location'] = result.get('location')

            return output

        except Exception as e:
            print(f"❌ Agent 2 failed: {e}")
            return {'price': None, 'city': None, 'location': None}